...
public class DynamoDBStore {
	...
	public void createTable(String tableName) {
		...
		keySchemaElement.add(new KeySchemaElement().withAttributeName("uniqueId").withKeyType(KeyType.HASH));
	...
	public Map<String, AttributeValue> newItem(String uniqueId, String itemName, String parentId, String blobId, 
	    ...
	    newItem.put("UniqueId", new AttributeValue().withS(uniqueId));
	    newItem.put("EntityName", new AttributeValue().withS(itemName));
	    newItem.put("ParentId", new AttributeValue().withS(parentId));
	    newItem.put("BlobId", new AttributeValue().withS(blobId));
	    newItem.put("FileSize", new AttributeValue().withN(Integer.toString(fileSize)));
	    newItem.put("ContentType", new AttributeValue().withS(contentType));
	    newItem.put("CreatedDate", new AttributeValue().withS(createdDate.toString()));
	    newItem.put("ModifiedDate", new AttributeValue().withS(modifiedDate.toString()));
        return newItem;
	}
	 ...
	 */
	public List<IEntity> getChildren(IFolder parent, String tableName, String fieldFilder, Condition condition) throws ParseException {
        ...
        for (Map<String, AttributeValue> item : items) {
            String blobId = item.get("BlobId").getS();
            String entityName = item.get("EntityName").getS();
            Date createdDate = dateFormat.parse(item.get("CreatedDate").getS());
            Date modifiedDate = dateFormat.parse(item.get("ModifiedDate").getS());
            if (StringUtils.isEmpty(blobId)) {
                ...
                file.setBytes(item.get("BlobId").getS().getBytes());
                file.setContentType(item.get("ContentType").getS());
                file.setSize(new Long(item.get("FileSize").getN()));
                childrens.add(file);
            }
        }
	 ...
	 * @return TRUE/FALSE
	 */
	public boolean isRootFolderCreated(String tableName, String fieldFilder, Condition condition) {
	    List<Map<String, AttributeValue>> items = getItem(tableName, fieldFilder, condition);
	    if (items.isEmpty())
	        return false;
	    return true;
	...
	private List<Map<String, AttributeValue>> getItem(String tableName, String fieldFilder, Condition condition) {
        HashMap<String, Condition> scanFilter = new HashMap<String, Condition>();
        scanFilter.put(fieldFilder, condition);
